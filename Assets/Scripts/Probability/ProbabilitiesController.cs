...
namespace Probability
{
    public static class ProbabilitiesController
    {
        ...
        public static int GetItemByProbabilityRarity(List<float> probabilityRarity)
        {
            ...
            return GetRandomIndex(probabilityRarity); //return -1 if some error happens
        }
        ...
        {
            UpdateNewSeedRealtime();
            return Random.Range(0, probabilities.Count);
        }

        private static void UpdateNewSeedRealtime()
        {
            DateTimeOffset currentTime = new DateTimeOffset(DateTime.UtcNow);
            long currentUnixTimestamp = currentTime.ToUnixTimeSeconds();
            int realtimeSeed = (int)(currentUnixTimestamp % int.MaxValue);
            Random.InitState(realtimeSeed);
        }
    }
}